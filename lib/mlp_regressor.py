...
class MLPRegressor_(BaseRegressor):
    grid_search_cv_params = {
        "n_jobs": -1,
        "cv": None,
        "verbose": 3,
    }
    mlp_regressor_params = {"hidden_layer_sizes": (100,), "activation": "relu", "solver": "adam", "max_iter": 200}

    def __init__(self, X=None, y=None, train_test_split_params=None, grid_search_cv_params=None, mlp_regressor_params=None):
        ...
        if mlp_regressor_params is not None:
            self.mlp_regressor_params = mlp_regressor_params

    def grid_search(self, X_train=None, y_train=None):
        logger.info(f"Start: Grid Search")
        logger.info(f"Grid Search Params: {self.mlp_regressor_params}")
        ...
        gscv = GridSearchCV(MLPRegressor(), self.mlp_regressor_params, **self.grid_search_cv_params)
